...
def load_reference():
    ...
    for row in range(5, 29):  # periods 1..24
        period = row - 4
        ref[period] = {
            "start": ws[f"DF{row}"].value,     # mites at start of period
            "end": ws[f"DS{row}"].value,       # mites at end of period
            "r": ws[f"DT{row}"].value,         # net r observed for the period
            "wash": ws[f"DX{row}"].value,      # alcohol-wash count
            "invaded": ws[f"CU{row}"].value,   # % worker cells invaded
            "phoretic": ws[f"CA{row}"].value,  # % mites phoretic
        }
    return ref
...
def main():
    ...
    for pr in run.periods:
        r = ref[pr.period]
        err = max(abs(pr.mites_start - r["start"]), abs(pr.mites_end - r["end"]),
                  abs(pr.wash_count - r["wash"]))
        max_err = max(max_err, err)
        ok = err < TOL
        all_ok &= ok
        print(f"{pr.period:>2} {pr.mites_start:>12.6f} {r['start']:>12.6f} "
              f"{pr.mites_end:>12.6f} {r['end']:>12.6f} "
              f"{pr.wash_count:>10.4f} {r['wash']:>10.4f}  {'OK' if ok else 'MISMATCH'}")
